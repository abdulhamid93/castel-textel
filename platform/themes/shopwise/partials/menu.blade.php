<ul {!! $options !!}>
    ...
    @foreach ($menu_nodes as $key => $row)
        <li @if ($row->has_child || $row->css_class || $row->active) class="@if ($row->has_child)  @endif @if ($row->css_class) {{ $row->css_class }} @endif @if ($row->active) active @endif" @endif>
            ...
            @if ($row->has_child)
                {{--<div class="dropdown-menu dropdown dropdown-reverse ">--}}
                    {!! Menu::generateMenu([
                        'menu'       => $menu,
